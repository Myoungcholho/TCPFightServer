...
<table style="border-collapse:collapse;">
  ...
  <tr>
    <td width="350" style="border:2px solid #ffb3b3; background:#ffe1e1; vertical-align:top;">
      트래픽으로 발생한 지연 문제를 트래픽 감소와 서버 유효성 검사로 해결했습니다.
      <br><br>
      <a href="#t0">[상세설명]</a>
    </td>
...
<table>
  <tr>
    <td style="border:2px solid #4fa3ff; border-radius:8px; padding:12px 16px; background:#050812;">
      <strong>🧩 문제</strong>
      <ul>
        <li> 이동 좌표를 지속적으로 송신하면서 패킷이 빠르게 연속으로 들어와 패킷 분석 난이도가 크게 증가하는 문제가 발생했음. </li>
      </ul>
      <strong>🔍 원인 분석</strong>
      <ul>
        <li> 플레이어 좌표는 움직이기 시작할 때와 멈출 때 정도에만 송신해도 되는데, 매 프레임 좌표를 보내는 구조라 불필요한 트래픽과 헤더 오버헤드가 과도하게 증가함 </li>
      </ul>
      <strong>🛠 해결</strong><br>
      <ul>
      ...
      <strong>✅ 결과</strong><br>
      <ul>
        <li> 이벤트 기반 전송으로 의미 없는 패킷이 줄어 패킷 흐름을 분석하고 디버깅하는 과정이 훨씬 명확해짐. </li>
      </ul>
      <strong>📚 배운 점</strong>
      <ul>
...
<table>
  <tr>
    <td style="border:2px solid #a8ddff; border-radius:8px; padding:12px 16px; background:#050a12;">
      <strong>🧩 문제</strong><br>
      <ul>
        <li> 유저 삭제 시 크래시가 발생하는 문제 발생 </li>
      </ul>
      <strong>🔍 원인 분석</strong><br>
      <ul>
        <li> 다른 플레이어가 다른 유저를 참조하고 있는데 객체가 죽어버려서 접근했을 때 크래시가 발생했던 것 </li>
        <li></li>
      </ul>
      <strong>🛠 해결</strong>
      <ul>
        <li> 삭제가 필요한 객체를 즉시 삭제하지 않고 배열에 기록해 모아둠 </li>
